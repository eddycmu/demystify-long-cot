...
def split_experience_batch(experience: Experience) -> List[BufferItem]:
    ...
    for k, v in experience.info.items():
        vals = torch.unbind(v)
        assert batch_size == len(vals), f"key: {k}, vals: {v.shape}, batch_size: {batch_size}"
        for i, vv in enumerate(vals):
            if k != "reward" and isinstance(vv, torch.Tensor):
...
def zero_pad_sequences(sequences: List[torch.Tensor], side: str = "left") -> torch.Tensor:
    ...
    for seq in sequences:
        ...
        if seq.dim() == 2:
            ...
            assert seq.dim() == 1, f"Expected seq dim to be 1 or 2, instead got {seq.dim()}"
...
def make_experience_batch(items: List[BufferItem], packing_samples=False) -> Experience:
    ...
    for key in items[0].info.keys():
        if key == "reward" and items[0].info["reward"].dim() >= 1:
            vals = zero_pad_sequences([item.info[key] for item in items], "left")
        else:
